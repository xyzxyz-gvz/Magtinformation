...
def fetch_cases(
    ...
    total = write_records(out_dir / "cases.json", normalized, merge=since is not None)
    logger.info("cases.json: %d new/updated, %d on disk", len(normalized), total)
...
def fetch_case_steps(
    ...
    normalized = normalize_records(records)
    total = write_records(out_dir / "case_steps.json", normalized, merge=since is not None)
    logger.info("case_steps.json: %d new/updated, %d on disk", len(normalized), total)
...
def fetch_case_actors(
    ...
    normalized = normalize_records(records)
    total = write_records(out_dir / "vote_case_actors.json", normalized, merge=since is not None)
    logger.info("vote_case_actors.json: %d new/updated, %d on disk", len(normalized), total)
